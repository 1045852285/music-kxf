<template>
  <div class="music-list">
    ...
    <!-- 为了正确计算scroll的高度，我们把songs作为data的数据传进去 -->
    <scroll
      @scroll="scroll"
      :probeType="probeType"
...
export default {
  ...
  watch: {
    scrollY(newY) {
      ...
      let scale = 1;
      let blur = 0;
      // 因为this.minTranslateY是个负数，所以最大距离滚动的就是this.minTranslateY的值
      ...
      if (newY > 0) {
        // 因为下面调用的scale方法里面的值是缩放的倍数
        // this.imageHeight乘以1 + precent等于this.imageHeight + newY
        // 图片的高度就增加了newY的高度，就是一个无缝滚动的了
        scale = 1 + precent;
        zIndex = 10;
      } else {
      ...
      // scale（x，y）x表示元素沿着水平方向（X轴）缩放的倍数，y表示元素沿着垂直方向（Y轴）缩放的倍数。
